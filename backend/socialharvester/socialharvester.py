"""
File: socialharvester.py
Author: Yechen Deng
Date: 20/05/2024
Description: This program harvests toots from aus.social after its first running - the first harvested toot time
"""
import sys
import json
import re
...
def update_state(prefix, max_id, record_count, cumulative_counts_1,es):
    ...
    try:
        response = es.index(index="aussocialstate", id=document_id, document=state_document)
...
def main():
    ...
    index_name = f"{SERVER2.replace('.', '')}"
    if token1 is None:
        current_app.logger.error(f"Error: No token found for server {SERVER2}")

    try:
        current_app.logger.info(f"Start fetching {SERVER2}")
        count = get_timelines(token1, f"https://{SERVER2}", N_YEARS, es)
        current_app.logger.info(f"Fetched {count} posts from {SERVER2}")
    except Exception as e:
        current_app.logger.error(f"Error processing {SERVER2}: {str(e)}")
